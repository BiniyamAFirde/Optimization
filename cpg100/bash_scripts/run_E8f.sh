#!/bin/bash -l
#SBATCH --job-name=cpg_E8f
#SBATCH --output=slurm_E8f_%A_%a.out
#SBATCH --error=slurm_E8f_%A_%a.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --mem=2G
#
# run_E8f.sh -- roadmap node E8f: final recipe, confirmed on 8 NEW seeds
# ==========================================================================
#
#   bash run_E8f.sh check             files + python packages + NEST present?  (run first)
#   bash run_E8f.sh plan              cells, task count (no NEST)
#   bash run_E8f.sh smoke             5 s, final recipe at 780 ms, pass A then B
#   bash run_E8f.sh --local [A|B|all] everything serially (Mac)        ~30-40 min
#   bash run_E8f.sh submit            Slurm: pass-A array, pass-B array (afterany)
#   bash run_E8f.sh summarize         gait-by-gait verdict vs the E8 k=1 baselines
#
#   The k=1 baselines are NOT re-run: results_E8/passB/base_k100_g*_stdpoff_* are
#   linked into results_E8f/passB (needs results_E8/ next to this script).
#
# -- WHY ----------------------------------------------------------------------
#   E5b (Mac, 60 s, 8 seeds): with 4 inputs per neuron (2,712 synapses) every gait
#   passes A1-A8, but not with one RG-F setting:
#     n100_Bk4     (RG-F c -55, d 4)  350 PASS -0.926+-0.018 | 520 PASS -0.980 | 780 A8 (RG -0.891)
#     n100_Bk4_d2  (RG-F c -50, d 2)  350 A3 (SD .040)      | 520 A6 (R -0.988) | 780 PASS -0.993
#   The final recipe is therefore gait-scheduled: RG-F less adapting at the slow gait.
#   Those cells were CHOSEN on seeds 12345..82345, so their pass is optimistic
#   (15 cells tried, best kept). E8f re-runs only the chosen recipe on 8 seeds
#   never used before; this is the number that goes in the write-up.
#   STDP-off copies show whether the result depends on STDP.
#
# -- GRID: 6 cells x 8 seeds x 2 passes = 96 runs, 60 s each -------------------
#   n100_final_g{350,520,780}_stdpon    final recipe, STDP 1e-6
#   n100_final_g{350,520,780}_stdpoff   same network, no STDP
#   Recipe: --preset n100 --conn-rule indegree --indeg-min 4 --indeg-min-conserve
#           --inh-comp 5.625; RG-F c -55 d 4 at 350/520 ms, c -50 d 2 at 780 ms.
#   Seeds: 13579 23579 ... 83579 (held out). Override with SEEDS="..." if needed.
#
# -- WHAT DECIDES THE NEXT NODE -----------------------------------------------
#   STDP-on cells pass A1-A8 at all 3 gaits  -> DONE (write-up with these numbers)
#   one gait fails on the new seeds           -> the E5b pass was seed luck: back to
#                                                E3/E5 at that gait (or E7 search)
#   STDP-off fails where STDP-on passes       -> report STDP as part of the recipe

set -uo pipefail

HERE="${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
PY="${PY:-python3}"
DRIVER="${DRIVER:-$HERE/cpg_e8f.py}"
SUMMARY="${SUMMARY:-$HERE/summarize_small.py}"
ROOT="${ROOT:-$HERE/results_E8f}"
...
CELLS=(
  "n100_final_g350_stdpon|n100|on|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --step-period-ms 350"
  "n100_final_g520_stdpon|n100|on|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --step-period-ms 520"
  "n100_final_g780_stdpon|n100|on|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --rgf-c -50 --rgf-d 2 --step-period-ms 780"
  "n100_final_g350_stdpoff|n100|off|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --step-period-ms 350"
  "n100_final_g520_stdpoff|n100|off|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --step-period-ms 520"
  "n100_final_g780_stdpoff|n100|off|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --rgf-c -50 --rgf-d 2 --step-period-ms 780"
)
say () { printf '\n\033[1m== %s\033[0m\n' "$*"; }
die () { printf '\n[FATAL] %s\n' "$*" >&2; exit 1; }
...
if [ -n "${CELLS_ONLY:-}" ]; then
  ...
  [ ${#_keep[@]} -gt 0 ] || die "CELLS_ONLY='$CELLS_ONLY' matches no cell (see: bash run_E8f.sh plan)"
  CELLS=("${_keep[@]}")
fi
...
if [ "${1:-}" != "check" ]; then
    for f in cpg_small.py cpg_e0.py cpg_e1.py cpg_e2.py cpg_e8f.py; do
        [ -f "$HERE/$f" ] || die "$f must be in $HERE -- run: bash run_E8f.sh check"
    done
fi
...
cmd_check () {
    ...
    local bad=0 f
    for f in cpg_small.py cpg_e0.py cpg_e1.py cpg_e2.py cpg_e8f.py run_E8f.sh summarize_small.py; do
...
PY
    if [ "$bad" = 0 ]; then echo; echo "  All good. Next: bash run_E8f.sh smoke"
    else echo; echo "  Not ready: conda activate nest, and put the files listed above in $HERE"; return 1; fi
}

cmd_plan () {
    say "E8f grid: $NCELL cells x $NSEED seeds = $NTASK tasks per pass (x2 passes), ${SIM_S}s each"
...
cmd_local () {
    local which=${1:-all} i
    if [ -z "${E8F_AWAKE:-}" ] && command -v caffeinate >/dev/null 2>&1; then
        echo "  (macOS: re-running under caffeinate -i so the Mac does not sleep mid-run)"
        E8F_AWAKE=1 exec caffeinate -i bash "$HERE/run_E8f.sh" --local "$which"
    fi
...
cmd_submit () {
    command -v sbatch >/dev/null || die "sbatch not found -- use: bash run_E8f.sh --local"
    mkdir -p "$ROOT"
    ...
    ja=$(sbatch --parsable -p "$PARTITION" --time="$TIME" --array=0-$((NTASK - 1)) \
         --export="$exp" "$HERE/run_E8f.sh" task A) || die "pass-A submit failed"
    jb=$(sbatch --parsable -p "$PARTITION" --time="$TIME" --array=0-$((NTASK - 1)) \
         --export="$exp" --dependency=afterany:"$ja" "$HERE/run_E8f.sh" task B) \
         || die "pass-B submit failed"
    echo "  pass A: job $ja   pass B: job $jb   ->  when B is done: bash run_E8f.sh summarize"
}

cmd_smoke () {
    local spec="n100_final_g780_stdpon|n100|on|indegree|0|--indeg-min 4 --indeg-min-conserve --stdp-lambda 1e-6 --inh-comp 5.625 --rgf-c -50 --rgf-d 2 --step-period-ms 780" x
    say "Smoke test: n100_final_g780_stdpon, 5 s, seed ${SEED_ARR[0]}, pass A then B"
    ROOT="$ROOT/smoke" SIM_S=5 FORCE=1
    mkdir -p "$ROOT"; run_one A "$spec" "${SEED_ARR[0]}" "$X0F_STOCK" "$X0F_STOCK" || die "pass A failed"
    x=$(prescribe n100_final_g780_stdpon) || die "prescription failed"
    run_one B "$spec" "${SEED_ARR[0]}" "${x% *}" "${x#* }" || die "pass B failed"
    "$PY" "$SUMMARY" --root "$ROOT" --ref-prefix ctrl_n480 --prefix "$ROOT/E8f_summary" >/dev/null \
...
cmd_summarize () {
    ...
    "$PY" "$SUMMARY" --root "$ROOT" --ref-prefix base_k100 --prefix "$ROOT/E8f_summary" \

...
# -*- coding: utf-8 -*-
"""
cpg_2legs_k020_m035.py
Reduced 2-leg CPG: k=0.20 synaptic connectivity, m=0.35 neuron count
"""
...
# both r_rge/100 and r_rgf/100 in the activation-gate sigmoid. 

ACT_GATE_K     = 8.0   # steepness for E's gate 
ACT_GATE_K_F   = 15.0  # NEW -- steeper slope 
ACT_GATE_X0_E  = 0.6   # unchanged
ACT_GATE_X0_F  = 1.30  
...
def main():
    ...
    ap.add_argument("--act-gate-x0-f", type=float, default=1.3)
    ap.add_argument("--act-gate-k-f", type=float, default=28.0)
    ...
    ap.add_argument("--k-conn", type=float, default=0.2)
    ap.add_argument("--m-neurons", type=float, default=0.35)
    ap.add_argument("--act-gate-x0-e", type=float, default=1.02)
    ap.add_argument("--act-gate-k-e", type=float, default=15.0)
    ap.add_argument("--inh-comp", type=float, default=20.0)
    ap.add_argument("--inh-comp-f", type=float, default=1.0)
    ap.add_argument("--w-ine2rgf", type=float, default=None)
    ap.add_argument("--w-inf2rge", type=float, default=None)
    ap.add_argument("--i-e-rgf", type=float, default=None)
    ap.add_argument("--w-rg-rec-f", type=float, default=None)
    ...
    if True:
        ...
        P_COMM_F      = 0.22*_k
        P_COMM_E      = 0.10*_k
    print(f"[net] k={_k} m={_m}  N_RG_E={N_RG_E} N_INE={N_INE} N_MOTOR_E={N_MOTOR_E}"
    ...
    if args.long_run:
        if args.simulate_chunk_ms<100.: args.simulate_chunk_ms=100.
        if args.rate_update_ms<100.:    args.rate_update_ms=100.
    ...
    print(f"[seed] NEST kernel rng_seed={run_seed} threads={args.threads}")
    ...
    # Connect  (CUT+BS->RG all static_synapse) 
    ...
    for side in LEGS:
        L=leg[side]
        S(L["cut_in"],L["rg_e"],P_IN_STDP,W0_IN,DL["cut_to_rg"])
        S(L["bs_in_e"],L["rg_e"],P_IN_STDP,W0_IN,DL["bs_to_rg"])
        S(L["bs_in_f"],L["rg_f"],P_IN_STDP,W0_IN,DL["bs_to_rg"])
    ...
    with h5py.File(args.out,"w") as h5:
      try:
        ...
        h5.attrs["stdp_lambda"]=0.0
        h5.attrs["fixed_weights"]=True
        ...
        for _nm in ("rge","rgf"):
            ...
            if _v.size:
                ...
                print(f"  [diag] RG-{_nm[-1].upper()} rate Hz: p1={_p1:.1f} p99={_p99:.1f} ptp={_p99-_p1:.1f}")
        for _nm,_key in (("de","act_e"),("df","act_f")):
            ...
            if _v.size:
                ...
                print(f"  [diag] gate {_nm}: p1={_p1:.3f} p99={_p99:.3f} ptp={_p99-_p1:.3f}")
        for _nm in ("force_e","force_f"):
            _v=np.asarray(_dg.get(_nm,[]),dtype=float)
            if _v.size:
                print(f"  [diag] {_nm}: p1={np.percentile(_v,1):.2f} p99={np.percentile(_v,99):.2f}")
